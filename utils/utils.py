...
def save_cache_to_drive(cache_path, dataset_name):
    try:
        print("Uploading dataset cache to Google Drive...")

        gauth = gg_auth()
        drive = GoogleDrive(gauth)

        drive_filename = f'cache_{dataset_name}.pkl'

        file_drive = drive.CreateFile({"title": drive_filename})
        file_drive.SetContentFile(cache_path)
        file_drive.Upload()

        print(
            f"Successfully uploaded cache to Drive with ID {file_drive.get('id')}")
        return True

    except Exception as e:
        print(
            f"Warning: Failed to upload cache to Google Drive. Error: {str(e)}")
        return False
...
if __name__ == "__main__":
    ...
    save_result_to_drive(epoch=1, path=path, prefix='epx',
                         score=score, save_checkpoint=save_checkpoint)
