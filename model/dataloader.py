...
from utils.utils import save_cache_to_drive


def get_dataset(tokenizer, dataset_cache, path, debug=False, debug_length=20):
    if dataset_cache and os.path.isfile(dataset_cache):
        print(f"Loading tokenized dataset from local cache at {dataset_cache}")
        data = torch.load(dataset_cache)
        return data

    # If no cache exists, create new dataset
    print(f"Creating new dataset from {path}")
    data = get_positional_dataset_from_file(
        tokenizer, file=path, debug=debug, debug_length=debug_length)

    if dataset_cache:
        # Save locally
        print(f"Saving dataset cache locally at {dataset_cache}")
        ...
        # Upload to Drive
        save_cache_to_drive(dataset_cache, dataset_name)

    print(f"Dataset cached at {dataset_cache}")
    return data
...
def get_positional_dataset_from_file(tokenizer, file, debug=False, debug_length=20):
    ...
    for inst in tqdm(data):
        ...
        total_seq_len = len(tokenized_para) + len(tokenized_answer) + len(
            tokenized_question) + len(tokenized_clue) + len(tokenized_qtype) + 6

        if total_seq_len > tokenizer.model_max_length:
            # Heuristic to chop off extra tokens in paragraphs
            tokenized_para = tokenized_para[:-1 *
                                            (total_seq_len - tokenizer.model_max_length + 1)]
            truncated_sequences += 1
            assert len(tokenized_para) + len(tokenized_answer) + len(tokenized_question) + \
                len(tokenized_clue) + len(tokenized_qtype) + \
                6 < tokenizer.model_max_length
        ...
        if clue_exist:
            ...
            clue_prefix_ids = tokenizer.convert_tokens_to_ids(
                tokenized_clue_prefix)
    ...
    print("%d / %d sequences truncated due to positional embedding restriction" %
