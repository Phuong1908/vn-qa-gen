...
def calculate_relative_positions(paragraph, answer_text, answer_start, clue_text=None, clue_start=None, max_length=512):
    """Calculate relative positions for each token in paragraph"""
    # Initialize positions array
    positions = []

    # Convert to character-level positions first
    for i in range(len(paragraph)):
        # Position relative to answer
        ans_relative_pos = i - answer_start if answer_start is not None else max_length

        # Position relative to clue
        clue_relative_pos = max_length
        if clue_text and clue_start is not None:
            if clue_start <= i < clue_start + len(clue_text):
                clue_relative_pos = 0  # Token is part of clue
            else:
                clue_relative_pos = min(abs(i - clue_start),
                                        abs(i - (clue_start + len(clue_text))))

        # Combine positions (using minimum distance)
        relative_pos = min(abs(ans_relative_pos), abs(clue_relative_pos))
        relative_pos = min(relative_pos, max_length - 1)  # Cap at max_length-1
...
def get_processed_examples(raw_examples, debug=False, debug_length=20, shuffle=True):
    ...
    for example in tqdm(raw_examples):
        ...
        relative_positions = calculate_relative_positions(
            ...
            clue_text=clue_text if clue_info["clue_text"] is not None else None,
            clue_start=clue_start if clue_info["clue_text"] is not None else None
        )
